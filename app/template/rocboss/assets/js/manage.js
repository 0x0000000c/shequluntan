...
function editTopicForm(tid) {
    $.post(root + 'manage/getTopicInfo', {
        ...
        layer.open({
            ...
            content: '<div id="post-edittopic"><form id="talk-add" class="add-post"><input type="hidden" name="tid" id="tid" value="' + data.tid + '"><div class="bz-form bz-form-stacked"><fieldset><label for="name">标题：</label><input type="text" name="title" id="title" class="bz-input-1" placeholder="（必填）请输入简要标题" value="' + data.title + '"><label for="name">帖子详情：</label><textarea id="subject" name="subject" rows="6"></textarea><label for="name">标题：</label><div id="tagInput" class="textarea"><div class="text-tag"><div id="tags" class="show-tag"><span class="tag" name="'+ data.tag+'">'+ data.tag+'</span></div><div class="clear"></div><input type="text" class="bz-input-1 form-tag-input" placeholder="（必填）请输入标签并回车显示，最多支持5个标签，点击标签以删除"> <input type="text" id="final" style="display:none" value="'+ data.tag+'"></div></div><label></label><button class="bz-button bz-button-primary" id="edit-btn" type="button" onclick="javascript:postEditTopic();">提交</button></fieldset></div></form></div><div class="clear"></div>'
        });
        var editor = new Simditor({
            textarea: $('#subject'),
            defaultImage: root + 'app/template/rocboss/assets/img/default.jpg'
