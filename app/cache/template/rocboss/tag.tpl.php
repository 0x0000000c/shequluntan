    ...
    <div class="main-outlet container">
        <div class="content left">
            <h2 class="nav-head" style="color: #017e66">
                <i class="icon icon-tag x2"></i> <?php echo $tagName ?>
            </h2>
            <h2 class="nav-head">
            <?php if ($loginInfo['uid'] > 0 ) { ?>
                <?php if ($signStatus == false) { ?>
                    <a href="javascript:doSign();" class="btn-circle right ml10" tip-title="今日签到"><i class="icon icon-squarecheck x2"></i></a>
                <?php } ?>
                <a href="javascript:showTopicForm2('<?php echo $tagName ?>');" class="btn-circle right" tip-title="发布帖子"><i class="icon icon-edit x2"></i></a>
            <?php } ?>
            <?php if (isset($_COOKIE['type']) && $_COOKIE['type'] == 'lasttime') { ?>
                <a href="<?php echo $root ?>do/posttime/" class="btn-circle" tip-title="按最新发表排序显示"><i class="icon icon-order x2"></i></a>
            <?php } ?>
            <?php if (isset($_COOKIE['type']) && $_COOKIE['type'] != 'lasttime') { ?>
                <a href="<?php echo $root ?>do/lasttime/" class="btn-circle" tip-title="按最后回复排序显示"><i class="icon icon-order x2"></i></a>
            <?php } ?>
            </h2>
            <?php include $this->_include('_part_topic_list.tpl.php', __FILE__) ?>
        </div>
        <div class="side">
